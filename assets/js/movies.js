...
const heroBanner = function ({ results: getMovieList }) {
    ...
    // Build media scroll for movie lists.
...
const buildPageHeader = function () {
    ...
    const genreLink = function () {
        ...
        for (const [genreId, genreName] of Object.entries(genreList)) {
            ...
            link.setAttribute(
                "href",
                `./media-grid.html?linkType=genre&urlParam=with_genres=${genreId}&pageName=${genreName}&mediaType=movie`
            );
            // OnClick attribute used to create a media list when a genre link is clicked.
            link.setAttribute(
                "onClick",
                `getMediaGrid("genre", "with_genres=${genreId}", "${genreName}", "movie")`
            );
...
const buildMediaScroll = function (
    ...
    mediaScrollElem.innerHTML = `
        <div class="media-scroll-title-wrapper">
            ...
            <a href="./media-grid.html?linkType=${linkType}&urlParam=${path}&pageName=${title}&mediaType=movie" 
            onclick='getMediaGrid("${linkType}", "${path}", "${title}", "movie")' 
